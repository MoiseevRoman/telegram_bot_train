...
from utils import translate_from_rus_to_eng, calculate_goals, get_food_info, get_exercise_info
...
async def start_command(message: Message):
    await message.answer("Привет! Я помогу тебе рассчитать нормы воды и калорий. Начни с команды /set_profile.")
...
async def log_food(message: Message, command: CommandObject):
    ...
    users[user_id]['logged_calories'] = users[user_id].get('logged_calories', 0) + food_info

    await message.answer(
        f"Вы съели {query}, это {food_info} калорий.\n"
    )
...
async def log_workout(message: Message, command: CommandObject):
    ...
    users[user_id]['water_added_goal'] = users[user_id]['water_goal'] + 200 * (users[user_id]['activity'] // 30)
    users[user_id]['calories_added_goal'] = users[user_id]['calories_goal'] + 200 * (users[user_id]['activity'] // 30)
    users[user_id]['burned_calories'] = exercise_info
    users[user_id]['logged_calories'] = users[user_id].get('logged_calories', 0) - exercise_info
...
async def check_progress(message: Message):
    ...
    if user_id not in users:
        await message.answer("Сначала настройте ваш профиль с помощью команды /set_profile.")
        return
